...
description: 'Explore the beauty of the bifurcation diagram!'
...
Lately, I did a small, funny project. The goal was to visualize a bifurcation. The bifurcation I was going to plot is one of the most famous ones: Its the bifurcation for the logistic map: `xn+1 = r * xn * ( 1 - xn )`. But before I'm going to talk about the program, I want to exemplify some of the mathematical backgrounds, without diving too deep into dynamic systems and chaos.

First of all, bifurcation is a period-doubling, which occurs while changing the control parameter. In my case, the control parameter is r, and the bifurcation happens, like in many others, in a fixed ratio: the Feigbaum Constant. As you increase r by a tiny amount up to 4 you are going to see that at r = 3, the graph splits into 2. At r ≈ 3.44, the graph forks into a period of 4. And so on until r ≈ 3.5699, then the deterministic chaos starts.
...
Here we have the function again: `xn+1 = r * xn * ( 1 - xn )`. xn is the current population, but it doesn't represent it in absolute value, rather in a relative amount to the theoretical maximum. For example, we let xn be this year's rabbit population of 0.5. The theoretical highest population possible is 1, due to nature factors like remaining space or whatever you can imagine. Now I want to calculate the population for next year.

For that, I have to know r. In the case of population calculation, r is considered as the growth rate. If we would only use the parameter r, this would be a linear system. For `0 < r < 1`, the population decreases over time and goes extinct at some point. For `r > 1`, the population increases infinitely. In nature, this isn't the case. Every year rabbits die, du enemies or old age. The term `(1 - xn)` represents this exact reduction. It also transforms this equation into a dynamic system. You also might notice that if the theoretical maximum of 1 gets reached, the population will go extinct next year.

# Now we can have a look at a simple calculation example.

Let's take a population of 0.4 with a growth rate of 2. If we put these values into the equation wwe will have a population of `x = 0.48` in the next year. A year later 0.4992 and if you iterate over, and over, you will see, that the population stays at about 0.5, that means, the equilibrium population is 0.5. That's the case for `1 <= r < 3`, not 0.5 every time, but the equilibrium population will settle down at one value.

> TODO: bild

At `r = 3`, the first period-doubling happens. Now the equilibrium population alternates between 2 values back and forth.

> Bild

With `r ≈ 3.44`, you are going to have a period of 4, the equilibrium population alternates between 4 values.

> 2 Bilder

And for the grand final after `r ≈ 3.5699`, you have chaos.
